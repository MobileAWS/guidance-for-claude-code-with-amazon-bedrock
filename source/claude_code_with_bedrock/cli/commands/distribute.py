...
class DistributeCommand(Command):
    ...
    # Optional Codex configuration — present only when the Nexus Lambda generated it
    _CODEX_CONFIG_FILE = "codex-config.json"
    ...
    def _create_per_os_archives(self, package_path: Path) -> list[tuple[str, Path]]:
        ...
        for platform, pconfig in self.PLATFORM_FILES.items():
            ...
            with zipfile.ZipFile(archive_path, "w", zipfile.ZIP_DEFLATED) as zf:
                ...
                # Include codex-config.json when present (generated by Nexus Lambda)
                codex_cfg = package_path / self._CODEX_CONFIG_FILE
                if codex_cfg.exists():
                    zf.writestr(
                        f"claude-code-package/{self._CODEX_CONFIG_FILE}",
                        self._read_file_with_retry(codex_cfg),
                    )
