...
def _get_all_templates() -> list[Path]:
    ...
    # Non-CloudFormation JSON config files that live alongside the templates
    non_template_json = {"nexus-transform-mcp.json"}
    templates = []
    if TEMPLATES_DIR.exists():
        templates.extend(TEMPLATES_DIR.glob("*.yaml"))
        templates.extend(TEMPLATES_DIR.glob("*.yml"))
        templates.extend(p for p in TEMPLATES_DIR.glob("*.json") if p.name not in non_template_json)
...
def test_cfn_lint_passes(template_path, cfn_lint_available):
    ...
    if result.returncode != 0 and result.stdout.strip():
        ...
        try:
            ...
            actual_errors = [e for e in actual_errors if e.get("Rule", {}).get("Id") != "E2531"]
            if actual_errors:
